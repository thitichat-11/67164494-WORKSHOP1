...
export default function Orders() {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case "Processing": return "bg-[#FAEBD9] text-[#A6713B]";
      case "COMPLETED":    return "bg-[#EEF4ED] text-[#3C7741]";
      case "REFUNDED":  return "bg-[#E2F1E0] text-[#2D612A]";
      case "Cancelled":  return "bg-[#FAEAEA] text-[#A73937]";
      // default:           return "bg-gray-100 text-gray-500";
    }
  };

  // ── Mapping สถานะ: display (หน้าจอ) <-> db (backend) ──
  ...
  // ── เปลี่ยนสถานะคำสั่งซื้อ: อัปเดตหน้าจอทันที (optimistic) + บันทึกลง DB ──
  const handleStatusChange = async (order, newDisplayStatus) => {
    const orderId = order.order_id;
    if (!orderId) {
      showAlert("เกิดข้อผิดพลาด", "ไม่พบเลขคำสั่งซื้อ (order_id) ไม่สามารถอัปเดตสถานะได้", "alert");
      return;
    }
    ...
    setOrders((prev) =>
      prev.map((o) =>
        o.order_id === orderId
          ? { ...o, status: newDisplayStatus, order_status: newDisplayStatus }
          : o
      )
    );
    ...
    try {
      ...
      await adminApi.put(`/api/orders/${orderId}/status`, {
        status: displayToDbStatus[newDisplayStatus],
      });
    } catch (error) {
      console.error("Failed to update order status:", error);
      setOrders(prevOrders); // rollback ถ้าบันทึกไม่สำเร็จ
      showAlert("เกิดข้อผิดพลาด", "ไม่สามารถอัปเดตสถานะคำสั่งซื้อได้ กรุณาลองใหม่อีกครั้ง", "alert");
    } finally {
                  ...
                  <tr key={row.key} className="hover:bg-[#FAF9F6] transition-colors">
                    ...
                    <td className="py-4 px-6 text-center">
                      <select
                        ...
                        className={
                          "px-3 py-1 rounded-full text-xs font-medium border-0 outline-none cursor-pointer text-center " +
