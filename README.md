...
* **Frontend Library (โครงสร้างหน้าเว็บหลัก):**
  * **React:** ใช้สำหรับสร้างส่วนติดต่อผู้ใช้งาน (User Interface) ในรูปแบบ Single Page Application (SPA) เพื่อการเปลี่ยนหน้าเว็บที่รวดเร็ว ลื่นไหล ไร้รอยต่อ และช่วยให้การจัดการข้อมูล (State Management) ของระบบตะกร้าสินค้าหรือหน้า Dashboard ทำได้อย่างมีประสิทธิภาพ
* **CSS Frameworks & Styling (การตกแต่งหน้าจอและจัด Layout):**
  * **Tailwind CSS:** ใช้สำหรับการตกแต่ง UI สไตล์ Utility-first ช่วยให้การออกแบบระบบ Design System ของแบรนด์ SALA เป็นไปในทิศทางเดียวกัน และสามารถจัดการออกแบบหน้าจอให้รองรับทุกอุปกรณ์ (Responsive Design) ทั้งสมาร์ตโฟน แท็บเล็ต และคอมพิวเตอร์ได้อย่างรวดเร็ว
  * **Bootstrap:** ใช้ควบคู่ในส่วนของ Grid System ฟอร์มกรอกข้อมูล และ Component สำเร็จรูป (เช่น Modal, Dropdown, Alerts) เพื่อเพิ่มความเสถียรและความแม่นยำในการจัดโครงร่างหน้าเว็บและโครงสร้างพื้นฐานให้เป็นมาตรฐานเดียวกัน
...
* **Functional Testing (การทดสอบเชิงฟังก์ชัน):** เพื่อตรวจสอบว่าฟังก์ชันการทำงานหลักทั้งหมดของระบบ เช่น การสมัครสมาชิก, การค้นหาเสื้อผ้า, ระบบตะกร้าสินค้า, ระบบสั่งซื้อ, และการจัดการหลังบ้านของ Admin สามารถทำงานได้ถูกต้องตามเงื่อนไขและขอบเขตที่กำหนดไว้ (User Requirements & System Scope)
* **User Interface (UI) & Usability Testing:** เพื่อตรวจสอบการแสดงผลบนหน้าจอ ความสวยงาม ความถูกต้องของตัวอักษร และความลื่นไหลในการใช้งาน รวมถึงตรวจสอบความสามารถในการรองรับการแสดงผลบนอุปกรณ์ที่หลากหลาย (Responsive Design) ทั้งบนสมาร์ตโฟนและคอมพิวเตอร์

### 9.2 เครื่องมือที่ใช้ในการทดสอบ (Testing Tools)
* **Manual Testing (การทดสอบด้วยตนเอง):** ในโปรเจกต์นี้เลือกใช้วิธีการทดสอบแบบแมนนวล โดยสมาชิกในกลุ่มร่วมกันจำลองสถาณการณ์และบทบาทสมมติ (User Roles) เป็นทั้ง "ลูกค้า (Customer)" และ "ผู้ดูแลระบบ (Admin)" เพื่อทดลองใช้งานระบบผ่านเว็บเบราว์เซอร์ (เช่น Google Chrome, Safari) แล้วบันทึกผลลัพธ์ว่าตรงตามที่ออกแบบไว้หรือไม่ โดยไม่มีการใช้เครื่องมือจำพวก Automated Test Scripts

### 9.3 รายละเอียดการทดสอบ (Testing Details)
การทดสอบจะถูกแบ่งออกเป็นกรณีทดสอบ (Test Cases) ตามฟังก์ชันสำคัญของระบบ ดังนี้:
* **Test Case 1: ระบบจัดการสมาชิกและการเข้าสู่ระบบ (Authentication)**
  * ทดสอบการสมัครสมาชิกด้วยข้อมูลที่ถูกต้อง และทดสอบระบบตรวจสอบกรณีรหัสผ่านไม่ตรงกัน
  * ทดสอบการล็อกอินเข้าสู่ระบบด้วยสิทธิ์ Customer และ Admin เพื่อเช็กความถูกต้องในการเข้าถึงหน้าเพจที่ต่างกัน
* **Test Case 2: ระบบค้นหาและเลือกดูสินค้าเสื้อผ้า (Product Search & View)**
  * ทดสอบการกรอกคำค้นหา (Keywords) และการใช้ตัวคัดกรอง (Filter) ตามหมวดหมู่ ไซส์ หรือราคา
  * ตรวจสอบว่าระบบแสดงรายละเอียดเสื้อผ้า รูปภาพ ราคา และจำนวนสต็อกได้อย่างถูกต้อง
* **Test Case 3: ระบบตะกร้าสินค้าและการสั่งซื้อ (Cart & Checkout)**
  * ทดสอบการเพิ่มเสื้อผ้าลงตะกร้า การเพิ่ม-ลดจำนวนสินค้าในตะกร้า และการคำนวณราคารวมทั้งหมด
  * ทดสอบขั้นตอนกรอกที่อยู่จัดส่ง และการกดชำระเงินจำลอง (Simulation) เพื่อตรวจสอบว่าสต็อกสินค้าในคลังลดลงโดยอัตโนมัติอย่างถูกต้อง
* **Test Case 4: หน้าจัดการหลังบ้านและรายงานสถิติ (Admin Dashboard)**
  * ทดสอบการเพิ่ม แก้ไข และลบข้อมูลเสื้อผ้า/หมวดหมู่จากฝั่ง Admin ว่าข้อมูลอัปเดตไปยังหน้าแรกทันทีหรือไม่
  * ตรวจสอบหน้า Dashboard ว่าสามารถแสดงสถิติกราฟยอดขายและอันดับเสื้อผ้าขายดีได้อย่างแม่นยำ
